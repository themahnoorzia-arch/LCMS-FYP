...
function JudgeDashboard() {
  ...
  const [remarksHearing, setRemarksHearing] = useState(null);
...
useEffect(() => {
    ...
    try {
      ...
      if (result.hearings) {
        setHearings(result.hearings.map(h => ({
          ...
          remarks: h.remarks || '',
          hearingstatus: h.hearingstatus || 'scheduled',
        })));
      }
    } catch (err) {
  ...
  const handleViewHistory = async (case_) => {
    ...
    setCaseHistory(history.length > 0 ? history : (case_.history || []));
  ...
  const openHearingForCase = (case_) => {
    const scheduled = hearings.find(h =>
      Number(h.caseid) === Number(case_.id) && h.hearingstatus === 'scheduled'
    );
    setEditingHearing(scheduled || null);
    setHearingForm({
      caseId: case_.id,
      caseTitle: case_.title,
      courtName: case_.courtName,
      hearingDate: scheduled?.hearingdate || '',
      hearingTime: scheduled?.hearingtime || '',
    });
    setShowAddHearingModal(true);
  };

  const openNewHearing = () => {
    setEditingHearing(null);
    setHearingForm({ caseId: '', caseTitle: '', courtName: '', hearingDate: '', hearingTime: '' });
    setShowAddHearingModal(true);
  };

  const handleHearingCaseSelection = (e) => {
    const selected = cases.find(case_ => Number(case_.id) === Number(e.target.value));
    setHearingForm(prev => ({
      ...prev,
      caseId: selected?.id || '',
  ...
  try {
    ...
    const savedHearing = {
        id: result.hearing?.hearingid || editingHearing?.id,
        caseid: hearingForm.caseId,
        casetitle: hearingForm.caseTitle,
        courtname: hearingForm.courtName,
        hearingdate: hearingForm.hearingDate,
        hearingtime: hearingForm.hearingTime,
        remarks: editingHearing?.remarks || '',
        hearingstatus: 'scheduled',
      };
    setHearings(prev => editingHearing
      ? prev.map(h => h.id === editingHearing.id ? { ...h, ...savedHearing } : h)
      : [...prev, savedHearing]);
...
const updateHearingRemarks = async (hearingId, remarks) => {
 ...
 const handleAddRemarks = (hearing) => {
  setRemarksHearing(hearing);
  setRemarksText(hearing.remarks || '');
  setShowRemarksModal(true);
};

 const handleRemarksSubmit = async (event) => {
  event.preventDefault();
  if (!remarksHearing) return;
  setSavingRemarks(true);
  const success = await updateHearingRemarks(remarksHearing.id, remarksText.trim());
  setSavingRemarks(false);
  if (!success) return;
  setHearings(current => current.map(hearing =>
    hearing.id === remarksHearing.id
      ? { ...hearing, remarks: remarksText.trim() }
      : hearing
  ));
  setShowRemarksModal(false);
  setRemarksHearing(null);
  setRemarksText('');
};
          ...
          <Col lg={8}>
            <Card className="border-0 shadow-sm h-100">
              <Card.Header className="bg-white border-bottom d-flex justify-content-between align-items-center">
                ...
                <Button size="sm" className="d-flex align-items-center gap-2" onClick={openNewHearing}>
                        ...
                        <tr key={hearing.id}>
                          ...
                          <td>
                            <Button variant="outline-primary" size="sm" className="me-1" onClick={() => {
                              const case_ = cases.find(c => Number(c.id) === Number(hearing.caseid));
                              if (case_) openHearingForCase(case_);
                            }}>
                              Update Hearing
                            </Button>
                            <Button variant="outline-warning" size="sm" onClick={() => handleAddRemarks(hearing)}>
  ...
  return (
    <div style={{ minHeight: '100vh', width: '100vw', overflow: 'hidden', background: '#f8f9fa', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Add Hearing Modal */}
      <Modal show={showAddHearingModal} onHide={() => { setShowAddHearingModal(false); setEditingHearing(null); }} centered>
        <Modal.Header closeButton>
          <Modal.Title>{editingHearing ? 'Update Hearing' : 'Schedule Hearing'}</Modal.Title>
        </Modal.Header>
        <Form onSubmit={handleAddHearing}>
          <Modal.Body>
            <Form.Group className="mb-3">
              ...
              {editingHearing ? (
                <Form.Control type="text" value={hearingForm.caseTitle} disabled />
              ) : (
                <Form.Select value={hearingForm.caseId} onChange={handleHearingCaseSelection} required>
                  <option value="">Select an assigned case</option>
                  {cases
                    .filter(case_ => !hearings.some(h =>
                      Number(h.caseid) === Number(case_.id) && h.hearingstatus === 'scheduled'
                    ))
                    .map(case_ => <option key={case_.id} value={case_.id}>{case_.title}</option>)}
                </Form.Select>
              )}
            </Form.Group>
            <Form.Group className="mb-3">
              <Form.Label>Court Name</Form.Label>
              <Form.Control type="text" value={hearingForm.courtName} disabled />
            </Form.Group>
            <Form.Group className="mb-3">
          ...
          <Modal.Footer>
            ...
            <Button type="submit" style={{ background: '#1ec6b6', border: 'none' }}>
      ...
      <Modal
        ...
        centered
      >
        <Modal.Header closeButton={!savingRemarks}>
          <Modal.Title>Edit Hearing Remarks</Modal.Title>
        </Modal.Header>
        <Form onSubmit={handleRemarksSubmit}>
          <Modal.Body>
            <div className="mb-3 p-3 rounded bg-light">
              <div className="fw-semibold">{remarksHearing?.casetitle}</div>
              <small className="text-muted">
                {remarksHearing?.hearingdate} at {remarksHearing?.hearingtime}
              </small>
            </div>
            <Form.Group>
          ...
          <Modal.Footer>
            ...
            <Button type="submit" disabled={savingRemarks || !remarksText.trim()}>
            ...
            <div key={entry.historyid || idx} className="mb-4" style={{ position: 'relative' }}>
              ...
              <div className="ps-2">
                <div className="d-flex align-items-center gap-2 mb-1 flex-wrap">
                  <span className="text-muted small">{entry.actionDate || entry.date || '—'}</span>
      ...
      >
        ...
        <Modal.Body>
          {!selectedCase?.witnesses?.length ? (
            <div className="text-center text-muted py-4">No witnesses are attached to this case.</div>
          ) : (
          <ListGroup variant="flush">
            {selectedCase.witnesses.map((witness, index) => (
              <ListGroup.Item key={index} className="border-0 mb-3 p-3 rounded-3 shadow-sm">
                ...
                <div className="mt-2">
                  <strong>Statement:</strong> {witness.statement || '—'}<br />
                  <strong>Statement Date:</strong> {witness.statementDate || '—'}
                </div>
              </ListGroup.Item>
            ))}
          </ListGroup>
      ...
      >
        ...
        <Modal.Body>
          {!selectedCase?.evidence?.length ? (
            <div className="text-center text-muted py-4">No evidence is attached to this case.</div>
          ) : (
          <ListGroup variant="flush">
